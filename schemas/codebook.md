...
투자자 마스터 테이블. Role 1이 확정한 7명의 메타데이터를 관리합니다.
...
| # | investor_id | Firm | CIK | Category | Style | Native Cadence |
|---|---|---|---|---|---|---|
| 1 | `INV_BUFFETT` | Berkshire Hathaway | 1067983 | Quality Compounder Value | `quality_value` | Annual letter (4Q carry-forward) |
| 2 | `INV_HAWKINS` | Southeastern Asset Management | 807985 | Deep Value Contrarian | `deep_value` | Semi-annual letter |
| 3 | `INV_GRANTHAM` | GMO | 1082609 | Macro-Value Allocator | `macro_value` | Quarterly |
...
| `Date` | `string` | `4Q08` | 텍스트가 귀속되는 분기. 형식: `NQYY` |
...
| Column | Type | Example | Description |
|---|---|---|---|
| `Date` | `string` (PK) | `1Q26` | 분기 식별자. 형식: `NQYY` (예: 1Q26) |
| `inflation` | `float` (0.0-1.0) | `0.73` | CPI YoY (CPIAUCSL) 기반 quantile (0~1) |
| `cycle` | `float` (0.0-1.0) | `0.14` | Real GDP Growth (GDPC1) 기반 quantile (0~1) |
| `Unemployment` | `float` (0.0-1.0) | `0.28` | Unemployment Rate (UNRATE, inverted) 기반 quantile (0~1) |
| `monetary` | `float` (0.0-1.0) | `0.76` | Fed Funds Rate (FEDFUNDS) 기반 quantile (0~1) |
| `stress` | `float` (0.0-1.0) | `0.63` | VIX (VIXCLS) 기반 quantile (0~1) |
| `macro state` | `string` (enum) | `Inflationary Tightening` | 경제 상태 라벨 (Recession, Expansion, Neutral, Stagflation Risk, Inflationary Tightening) |
...
| `Unemployment` | `UNRATE` | Monthly → Quarterly avg | Inverted level |
...
| Column | Type | Example | Description |
|---|---|---|---|
| `Date` | `string` (PK) | `3Q08` | 분기 식별자. 형식: `NQYY` |
...
|---|---|---|---|
| `Date` | `string` (PK) | All Roles | 분기 식별자 |
| `investor_id` | `string` (PK) | Role 1 | 투자자 식별자 (복합 PK) |
| `macro_state` | `string` | Role 2 | 5차원 quantile 벡터 (JSON array) — `[inflation, cycle, Unemployment, monetary, stress]` |
| `macro state` | `string` | Role 2 | 경제 상태 라벨 (예: `Expansion`) |
